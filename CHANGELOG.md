...
- **TabBar 预览切换按钮**：Markdown 编辑模式下，TabBar 右侧新增预览切换按钮，点击可开关预览面板，提升预览功能发现性
- **保存并返回阅读命令**：命令面板新增「保存并返回阅读」命令，编辑模式下可用，执行后保存当前文件并自动切回阅读模式
- **SourceViewer 搜索行定位**：全局内容搜索结果点击后，文本/代码文件可精确定位到匹配行并高亮 2 秒，提升伴读体验

### 变更

- **命令面板能力过滤**：命令面板隐藏当前不可用的命令（如非 MD 文件不显示「切换阅读/编辑模式」「保存并返回阅读」），替代原有的禁用置灰样式
- **伴读诚实化收尾**：补齐命令面板、快捷键、原生菜单等所有入口的能力过滤，非 MD 文件无任何编辑入口可见

### 测试

- 单元测试：515 个用例全部通过
- E2E 测试：新增 `editor-preview.spec.ts` 6 个用例（预览按钮、保存并返回阅读）、`surface-honesty.spec.ts` 1 个用例（命令面板过滤）、`source-viewer.spec.ts` 2 个用例（行定位）
