...
| 编辑 | CodeMirror 6 编辑器、Markdown 语法高亮、工具栏命令、自动保存、冲突检测、统一编辑会话状态机、编辑时预览面板 |
| 表格编辑 | 工具栏/命令面板插入可配置行列的表格、Tab/Shift-Tab 单元格导航、Enter 新增行、Backspace 删除空行 |
| 路径补全 | 输入 `[[`、`./`、`![](`、`](` 触发文件路径补全，支持模糊匹配与目录导航 |
| 浏览 | 递归文件树、隐藏文件灰色标注、可配置忽略列表与 Markdown 扩展名 |
| 标签 | 多标签切换/关闭、脏标记、惰性加载 |
| 伴读 | 文本/代码文件源码查看、语法高亮、行号显示、搜索高亮跳转 |
...
| Tab | 表格内跳到下一单元格（编辑模式） |
| Shift+Tab | 表格内跳到上一单元格（编辑模式） |
| Enter | 表格内在当前行下方新增行（编辑模式） |
| Backspace | 表格内删除当前空数据行（编辑模式） |
...
| View | Toggle Preview | Ctrl+Shift+P（命令面板） |
