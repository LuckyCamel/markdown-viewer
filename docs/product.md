...
| 类别       | 功能                                                                                                          |
| ---------- | ------------------------------------------------------------------------------------------------------------- |
| 渲染       | GFM 全套、KaTeX 数学、Mermaid 图表、highlight.js 代码高亮、大文档分片渲染（> 1000 行首屏 200 行 + 滚动追加）  |
| 编辑       | CodeMirror 6 编辑器、Markdown 语法高亮、工具栏命令、自动保存、冲突检测、统一编辑会话状态机、编辑时预览面板、保存并返回阅读 |
| 表格编辑   | 工具栏/命令面板插入可配置行列的表格、Tab/Shift-Tab 单元格导航、Enter 新增行、Backspace 删除空行               |
| 路径补全   | 输入 `[[`、`./`、`![](`、`](` 触发文件路径补全，支持模糊匹配与目录导航                                        |
| 浏览       | 递归文件树、隐藏文件灰色标注、可配置忽略列表与 Markdown 扩展名、按需懒加载子目录（loading 占位 + 空目录提示） |
| 大文件守护 | 打开超阈值文件前提示（Markdown 5MB / 文本 2MB）；二进制文件拒绝读取为文本                                     |
| 标签       | 多标签切换/关闭、脏标记、惰性加载                                                                             |
| 伴读       | 代码文件源码查看、25+ 编程语言语法高亮、行号显示、搜索行定位与高亮、搜索高亮跳转、文件类型分层 UX（MD 可编辑、代码只读） |
| 搜索       | 文件搜索 (Ctrl+P)、全局内容搜索 (Ctrl+Shift+F)、编辑器内查找替换 (Ctrl+F)、可配置文本扩展名（控制搜索范围） |
...
Release 由推送 `v*` tag 触发，详见 [AGENTS.md](../AGENTS.md#发布流程)。
