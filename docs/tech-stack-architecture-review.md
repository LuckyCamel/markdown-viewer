...
| 项 | 内容 |
|----|------|
| 评估对象 | Markdown Viewer（跨平台 Tauri 桌面 Markdown 阅读器） |
| 评估日期 | 2026-07-16 |
...
以**工作区**方式浏览本地 Markdown（及扩展的文本/代码）文件：GFM 渲染、KaTeX、Mermaid、多标签、文件树、全文搜索、大纲、主题、原生菜单、命令面板、导出、每日笔记等。定位是**阅读器 / 查看器**，不是完整 IDE 或编辑器。
...
| 层级 | 选型 | 版本方向 | 评价 |
|------|------|----------|------|
| 桌面壳 | **Tauri 2** | 2.x | 优秀：相对 Electron 体积小、内存低，契合本地文件阅读器 |
...
| 项 | 严重度 | 说明 |
|----|--------|------|
| **CodeMirror 依赖已引入但主路径未接入** | 中 | `package.json` 含多包 `@codemirror/*` 与 `lezer-markdown`；`src/renderer/lib/codemirror/` 有 extensions/theme，但 `SourceViewer` 仍用 highlight.js。增加安装体积与维护面，却未兑现编辑/源码能力 |
| **「不引入新依赖」约束偏紧** | 低–中 | 有利于控制膨胀，但会鼓励「半成品依赖残留」或复制实现；建议改为「新依赖须评审 + 清单记录」 |
| **Tailwind 停留在 v3** | 低 | 非阻塞 |
| **版本号文档不一致** | 低 | README 写 1.3.0，`package.json` 为 1.3.1，`product.md` 仍为 1.2.13 |

**栈层面结论**：核心选型与产品匹配度高，**无明显错误选型**；主要清理点是未使用的 CodeMirror 依赖与文档版本同步。
...
这与「本地文件阅读器」场景高度契合：**系统敏感能力在 Rust/插件侧，UI 与渲染在 Web 侧**。分层文档（`docs/architecture.md`）与实现大体一致。
...
对本地 Markdown 阅读器而言，安全模型** indus­try-solid**，是本项目架构亮点。
...
| 文档 | 作用 | 健康度 |
|------|------|--------|
| `architecture.md` | 架构真源 | 主体准确，**store 原则与模块表略滞后**（favorites、export、CodeMirror 半成品等） |
| `development.md` | 开发与测试策略 | 清晰 |
| `product.md` | 产品能力 | 版本号滞后 |
...
1. **收敛 CodeMirror**  
   - 若近期不做编辑：移除 `@codemirror/*`、`lezer-markdown` 与 `lib/codemirror`，坚持 highlight.js 源码视图。  
   - 若确定做编辑：明确 `SourceViewer`/`MarkdownEditor` 边界，接入后删 highlight 双路径。
...
3. **修正架构文档**  
   - store 协同规则改为与代码一致  
   - 补全 favorites、export、多根工作区、命令面板等模块表  
   - 统一版本号表述（README / product / package）

4. **可选：设置 key 常量化**  
   减少 `ipc.store` 字符串散落。

5. **可选：1–3 个真实 Tauri 冒烟用例**  
...
| 立刻上 Monaco 全家桶 | 体积与「阅读器」定位冲突；若编辑再评估 |
